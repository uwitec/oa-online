...
function addcom(){ 
var fullname = $ID("custom.fullName") ;  
if(fullname.value.trim()==""){   
  $ID("inpageinfo").innerHTML = "客户名称不能为空" ;
...
CustomDWR.hasCustom(fullname.value.trim(),"",function(data){
 if(data==true){ 
    $ID("inpageinfo").innerHTML = "客户名称已经存在" ;
    $ID("inpageinfo").style.display = "block" ;
    return ; 
 }else{
var address = $ID("custom.address") ;
if(address.value.trim()==""){ 
  $ID("inpageinfo").innerHTML = "客户地址不能为空" ;
  $ID("inpageinfo").style.display = "block" ;
  address.select();
  return ;
}
var checkworker = $ID("checkworker") ;
var workor = $ID("custom.workor").value.trim() ;
if(!checkworker.checked && workor==""){ 
  $ID("inpageinfo").innerHTML = "业务员不能为空" ;
  $ID("inpageinfo").style.display = "block" ; 
  return ;
}
/*
var postcode = $ID("custom.postcode") ;
...
var businessNo = $ID("custom.businessNo") ;
...
var nationTaxNo = $ID("custom.nationTaxNo") ;
...
var landTaxNo = $ID("custom.landTaxNo") ;
...
var cnnumber = $ID("custom.cnnumber") ;
...
CustomDWR.hasCustom(fullname.value.trim(),id,function(data){
 if(data==true){ 
    $ID("inpageinfo").innerHTML = "客户名称已经存在" ;
  $ID("inpageinfo").style.display = "block" ;
    return ; 
...
function selParentCom(){
  ...
  if(parent!=""){
     $ID("custom.parentId").value = parent.split("-")[0] ;
     $ID("custom.parentName").value = parent.split("-")[1] ; 
  } 
}
