...
TabPanel = function(config){
  ...
  this.width = config.width || '400px';//宽度
  this.height = config.height || '300px';//高度
  this.border = config.border;
  this.items = config.items;//选项卡
  this.active = config.active || 0;//激活哪个
  this.tabs = new Array();//选项卡数组
...
TabPanel.prototype = {
  ...
  init : function(){
    ...
    if(this.border=='none')
    {
      ...
      this.tabpanel_tab_content.css('width', (this.tabpanel_tab_content.get(0).offsetWidth)+'px');
      this.tabpanel_content.css('width', (this.tabpanel_tab_content.get(0).offsetWidth)+'px');
    }
    else
    {
      //计算选项卡承载层和内容层宽度(重点是要去除边框的宽度)
      this.tabpanel_tab_content.css('width', (this.tabpanel_tab_content.get(0).offsetWidth-4)+'px');
      this.tabpanel_content.css('width', (this.tabpanel_tab_content.get(0).offsetWidth-2)+'px');
    }
